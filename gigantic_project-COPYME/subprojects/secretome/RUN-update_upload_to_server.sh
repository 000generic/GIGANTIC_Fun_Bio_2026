#!/bin/bash
# AI: Claude Code | Opus 4.8 | 2026 July 11 | Purpose: Thin wrapper around shared update_upload_to_server.py
# Human: Eric Edsinger

################################################################################
# GIGANTIC secretome - Upload to Server Builder
################################################################################
#
# PURPOSE:
...
# (STEP_1-build_evidence_table/workflow-RUN_* and
#  STEP_2-filter_secretome/workflow-RUN_* dirs), the shared helper:
#   - reads included files
#   - symlinks them into
#     upload_to_server/<STEP>/workflow-RUN_*/<N-output>/ preserving the
#     N-output/ subdirs (OUTPUT_pipeline/ collapsed out)
#   - publishes ONLY the most recent RUN per step (superseded runs pruned)
